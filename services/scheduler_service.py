...
def create_scheduled_job(job_name, description, target_type, target_id, action_type, payload_json, interval_minutes, auto_approve=0, only_when_online=0):
    conn = get_db()
    cur = conn.cursor()
    next_run = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")

    cur.execute("""
        INSERT INTO scheduled_jobs
        (job_name, description, target_type, target_id, action_type, payload_json, interval_minutes, auto_approve, only_when_online, is_enabled, next_run_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?)
    """, (
        (job_name or "").strip(),
...
def set_job_enabled(job_id, enabled):
    conn = get_db()
    cur = conn.cursor()
    cur.execute("UPDATE scheduled_jobs SET is_enabled = ? WHERE id = ?", (1 if enabled else 0, job_id))
    conn.commit()
    conn.close()

def queue_remote_command(cur, machine_id, action_type, payload_json, source="scheduler", approval_status="pending_approval"):
    try:
        cur.execute("""
            INSERT INTO remote_commands (machine_id, action_type, payload_json, status, source, created_at)
            VALUES (?, ?, ?, ?, ?, datetime('now'))
        """, (machine_id, action_type, payload_json, approval_status, source))
    except Exception:
        cur.execute("""
            INSERT INTO remote_commands (machine_id, action_type, payload_json, status, created_at)
            VALUES (?, ?, ?, ?, datetime('now'))
        """, (machine_id, action_type, payload_json, approval_status))
...
def run_job_now(job_id):
    ...
    for machine in machines:
        if job["only_when_online"] and not machine["is_online"]:
            skipped += 1
            continue
        queue_remote_command(cur, machine["id"], job["action_type"], job["payload_json"], "scheduler_manual", approval_status)
        queued += 1
    ...
    for job in jobs:
        ...
        for machine in machines:
            if job["only_when_online"] and not machine["is_online"]:
                skipped += 1
                continue
            queue_remote_command(cur, machine["id"], job["action_type"], job["payload_json"], "scheduler", approval_status)
            queued += 1

        next_run = datetime.utcnow() + timedelta(minutes=int(job["interval_minutes"] or 60))
        cur.execute("""
...
def get_scheduler_overview(cur=None):
    ...
    try:
        cur.execute("SELECT COUNT(*) AS c FROM scheduled_jobs WHERE is_enabled = 1")
        row = cur.fetchone()
        overview["enabled_jobs"] = row["c"] if row else 0

        cur.execute("SELECT COUNT(*) AS c FROM scheduled_jobs WHERE is_enabled = 0")
        row = cur.fetchone()
        ...
        cur.execute("""
            SELECT *
            FROM scheduled_jobs
            ORDER BY is_enabled DESC, next_run_at ASC, id DESC
            LIMIT 10
        """)
