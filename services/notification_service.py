...
def _discord_enabled(settings) -> bool:
    return (
        get_bool_setting(settings, "notifications_enable_discord", False)
        and bool((settings.get("discord_webhook_url") or "").strip())
    )
...
def notify_alert_resolved(
    ...
    notify_enabled = (
        get_bool_setting(settings, "notify_on_alert_resolve", True)
        or get_bool_setting(settings, "notify_on_alert_resolved", True)
    )
