...
def _prune_old_machine_data(cur, machine_id):
    cur.execute(
...
def report():
    ...
    ram = _safe_dict(data.get("ram"))
    disk = _safe_dict(data.get("disk"))
    network = _safe_dict(data.get("network"))
    disk_io = _safe_dict(data.get("disk_io"))
    inventory = _safe_dict(data.get("inventory"))
    gpu = _safe_dict(data.get("gpu"))

    drives = _safe_list(data.get("drives"))
    processes = _safe_dict(data.get("top_processes"))
    services = _safe_list(data.get("services"))
    interfaces = _safe_list(data.get("interfaces"))
    software = _safe_list(data.get("software"))
    gpus = _safe_list(data.get("gpus"))
    events = _safe_list(data.get("events"))
    ...
    try:
        ...
        cur.execute(
            ...
            (
                ...
                gpu.get("memory_total_mb"),
                gpu.get("temperature"),
                _safe_json_dumps(gpus) if gpus else None,
            ),
        )

        _prune_old_machine_data(cur, machine_id)
        ...
        for category in ("cpu", "memory"):
            for proc in _safe_list(processes.get(category)):
                process_rows.append(
                    (
                        machine_id,
                        now,
...
def command_result():
    ...
    try:
        ...
        log_event(
            ...
            "agent",
            _safe_json_dumps({"command_id": command_id, "output": output[:1500]}),
        )
