...
def machine_detail(machine_id):
    ...
    machine = get_machine_with_latest_snapshot(cur, machine_id)
    if not machine:
        conn.close()
        abort(404)

    cur.execute("SELECT * FROM snapshots WHERE machine_id = ? ORDER BY recorded_at DESC, id DESC LIMIT ?", (machine_id, MAX_RECENT_SNAPSHOTS))
    recent_snapshots = cur.fetchall()

    cur.execute("""
        SELECT * FROM drive_snapshots
        WHERE machine_id = ? AND recorded_at = (
            SELECT recorded_at FROM drive_snapshots WHERE machine_id = ? ORDER BY recorded_at DESC, id DESC LIMIT 1
        )
    ...
    cur.execute("""
        ...
        WHERE machine_id = ? AND recorded_at = (
            SELECT recorded_at FROM process_snapshots WHERE machine_id = ? ORDER BY recorded_at DESC, id DESC LIMIT 1
        ) AND category = 'cpu'
    ...
    cur.execute("""
        ...
        WHERE machine_id = ? AND recorded_at = (
            SELECT recorded_at FROM service_snapshots WHERE machine_id = ? ORDER BY recorded_at DESC, id DESC LIMIT 1
        )
    ...
    cur.execute("""
        ...
        WHERE machine_id = ? AND recorded_at = (
            SELECT recorded_at FROM interface_snapshots WHERE machine_id = ? ORDER BY recorded_at DESC, id DESC LIMIT 1
        )
    ...
    cur.execute("""
        ...
        WHERE machine_id = ? AND recorded_at = (
            SELECT recorded_at FROM software_snapshots WHERE machine_id = ? ORDER BY recorded_at DESC, id DESC LIMIT 1
        )
        ORDER BY name ASC
        LIMIT 300
    """, (machine_id, machine_id))
    ...
    cur.execute("SELECT * FROM alerts WHERE machine_id = ? AND is_resolved = 0 ORDER BY recorded_at DESC, id DESC", (machine_id,))
    ...
    chart_labels = [row["recorded_at"][-8:] if row["recorded_at"] else "" for row in snapshots_for_chart]
    ...
    temp_data = [None if row["cpu_temp"] is None else round(row["cpu_temp"], 2) for row in snapshots_for_chart]
